...
// A simple TFRectangle class that supplies intersection.
class TFRectangle {
 public:
  TFRectangle() { Set(0, 0, 0, 0); }
  TFRectangle(int xmin, int ymin, int xmax, int ymax) {
  ...
  TFRectangle Intersect(const TFRectangle& r) const {
    ...
    if (pmin_x > pmax_x || pmin_y > pmax_y) {
      ...
      return TFRectangle(pmin_x, pmin_y, pmax_x, pmax_y);
    }
  }
...
bool SatisfiesOverlapConstraints(const TFRectangle& crop,
                                 float minimum_object_covered,
                                 const std::vector<TFRectangle>& bounding_boxes) {
...
// Generate a random crop within the TFRectangle
...
// The diameter of the generated TFRectangle will be uniformly distributed between
// its minimum and maximum size. The center of the TFRectangle will be distributed
// uniformly within the source TFRectangle. The function returns false if the
// TFRectangle could not be generated with the given constraints.
bool GenerateRandomCrop(int original_width, int original_height,
                        ...
                        SimplePhilox* random, TFRectangle* crop_rect) {
  ...
  // Try first with a slightly bigger TFRectangle first.
  ...
  // Let us not fail if rounding error causes the area to be
  // outside the constraints.
  // Try first with a slightly smaller TFRectangle first.
  if (area > max_area) {
    height -= 1;
    width = static_cast<int>(lrintf(height * aspect_ratio));
    area = width * height;
...
class SampleDistortedBoundingBoxOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    std::vector<TFRectangle> bounding_boxes;
    if (input_boxes.NumElements() > 0) {
      ...
      for (int b = 0; b < boxes.dimension(0); ++b) {
        ...
        bounding_boxes.push_back(TFRectangle(x_min, y_min, x_max, y_max));
      }
    }

    // Insert the entire image if no bounding boxes are supplied.
    const TFRectangle image_rect(0, 0, width, height);
