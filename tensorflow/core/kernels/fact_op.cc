...
class FactOpKernel1 : public OpKernel {
 public:
  explicit FactOpKernel1(OpKernelConstruction* context) : OpKernel(context) {}
  ...
  void Compute(OpKernelContext* context) override {
    ...
    string coded = kFacts1[context->env()->NowMicros() % kNum1];
    E(&coded);
    output() = coded;
  }
};

class FactOpKernel2 : public OpKernel {
 public:
  explicit FactOpKernel2(OpKernelConstruction* context) : OpKernel(context) {}
  ...
  void Compute(OpKernelContext* context) override {
    ...
    string coded = kFacts2[context->env()->NowMicros() % kNum2];
    E(&coded);
    output() = coded;
  }
};
