...
#else
  int fd = open(translated_fname.c_str(), O_RDONLY | O_BINARY | O_RANDOM, S_IREAD);
#endif
  if (fd < 0) {
...
#else
  FILE* f = fopen(translated_fname.c_str(), "wb");
#endif
  if (f == NULL) {
    *result = NULL;
    s = IOError(fname, errno);
...
#else
  FILE* f = fopen(translated_fname.c_str(), "ab");
#endif
  if (f == NULL) {
    *result = NULL;
    s = IOError(fname, errno);
...
#else
  if (mkdir(TranslateName(name).c_str()) != 0) {
#endif
