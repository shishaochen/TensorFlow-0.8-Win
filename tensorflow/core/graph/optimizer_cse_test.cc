...
TEST_F(OptimizerCSETest, Simple) {
  InitGraph(
      ...
      " input: ['A', 'B'] }");
  EXPECT_EQ("A(Input);B(Input);D(Mul)|A->D;B->D:1", DoCSE());
}
...
TEST_F(OptimizerCSETest, Simple_ThreeEquivalent) {
  InitGraph(
      ...
      " input: ['A', 'B'] }");
  EXPECT_EQ("A(Input);B(Input);E(Mul)|A->E;B->E:1", DoCSE());
}
...
TEST_F(OptimizerCSETest, Simple_WithFixups) {
  InitGraph(
      ...
      "node { name: 'E' op: 'Mul' attr { key: 'T' value { type: DT_FLOAT } }"
      " input: ['C', 'D'] }");
  EXPECT_EQ("A(Input);B(Input);D(Mul);E(Mul)|A->D;B->D:1;D->E;D->E:1", DoCSE());
}
...
TEST_F(OptimizerCSETest, Simple_Commutative) {
  InitGraph(
      ...
      "node { name: 'D' op: 'Mul' attr { key: 'T' value { type: DT_FLOAT } }"
      " input: ['B', 'A'] }");
  EXPECT_EQ("A(Input);B(Input);D(Mul)|A->D:1;B->D", DoCSE());
}
...
TEST_F(OptimizerCSETest, SameConstants) {
  ...
  InitGraph(
      ...
      " input: ['A', 'B'] }");
  EXPECT_EQ("B(Const);D(Mul)|B->D;B->D:1", DoCSE());
}
...
TEST_F(OptimizerCSETest, DifferentConstants) {
  ...
  InitGraph(
      ...
      "node { name: 'D' op: 'Mul' attr { key: 'T' value { type: DT_INT32 } }"
      " input: ['A', 'B'] }");
  EXPECT_EQ("A(Const);B(Const);D(Mul)|A->D;B->D:1", DoCSE());
}
