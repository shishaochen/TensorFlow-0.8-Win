...
TEST(TestReporter, CreateTwiceFails) {
  string fname = io::JoinPath(testing::TmpDir(), "/test_reporter_dupe");
  {
    TestReporter test_reporter(fname, "t1");
    TF_EXPECT_OK(test_reporter.Initialize());
  }
  {
    TestReporter test_reporter(fname, "t1");
  ...
  Env::Default()->DeleteFile(fname + "t1");
}

TEST(TestReporter, CreateCloseCreateAgainSkipsSecond) {
  string fname = io::JoinPath(testing::TmpDir(), "/test_reporter_create_close");
  TestReporter test_reporter(fname, "t1");
  TF_EXPECT_OK(test_reporter.Initialize());
  TF_EXPECT_OK(test_reporter.Close());
...
TEST(TestReporter, Benchmark) {
  string fname =
      io::JoinPath(testing::TmpDir(), "/test_reporter_benchmarks_");
  ...
  Env::Default()->DeleteFile(expected_fname);
}
