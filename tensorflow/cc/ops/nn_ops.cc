...
Node* AvgPool3D(NodeOut input, gtl::ArraySlice<int> ksize, gtl::ArraySlice<int>
                ...
                opts) {
  if (opts.HaveError()) return nullptr;
  static const string kOpName = "AvgPool3D";
  NodeBuilder node_builder(opts.GetNameForOp(kOpName), kOpName,
                           opts.op_registry());
...
Node* AvgPool3DGrad(NodeOut orig_input_shape, NodeOut grad,
                    ...
                    {
  if (opts.HaveError()) return nullptr;
  static const string kOpName = "AvgPool3DGrad";
  NodeBuilder node_builder(opts.GetNameForOp(kOpName), kOpName,
                           opts.op_registry());
...
Node* BiasAddGrad(NodeOut out_backprop, const GraphDefBuilder::Options& opts) {
  static const string kOpName = "BiasAddGrad";
  return UnaryOp(kOpName, out_backprop, opts);
}

Node* BiasAddV1(NodeOut value, NodeOut bias, const GraphDefBuilder::Options&
                opts) {
  static const string kOpName = "BiasAddV1";
  return BinaryOp(kOpName, value, bias, opts);
}
...
Node* Conv3D(NodeOut input, NodeOut filter, gtl::ArraySlice<int> strides,
             StringPiece padding, const GraphDefBuilder::Options& opts) {
  if (opts.HaveError()) return nullptr;
  static const string kOpName = "Conv3D";
  NodeBuilder node_builder(opts.GetNameForOp(kOpName), kOpName,
                           opts.op_registry());
  node_builder.Input(input);
  node_builder.Input(filter);
...
Node* Conv3DBackpropFilter(NodeOut input, NodeOut filter, NodeOut out_backprop,
                           ...
                           const GraphDefBuilder::Options& opts) {
  if (opts.HaveError()) return nullptr;
  static const string kOpName = "Conv3DBackpropFilter";
  NodeBuilder node_builder(opts.GetNameForOp(kOpName), kOpName,
                           opts.op_registry());
  node_builder.Input(input);
  node_builder.Input(filter);
...
Node* Conv3DBackpropInput(NodeOut input, NodeOut filter, NodeOut out_backprop,
                          ...
                          const GraphDefBuilder::Options& opts) {
  if (opts.HaveError()) return nullptr;
  static const string kOpName = "Conv3DBackpropInput";
  NodeBuilder node_builder(opts.GetNameForOp(kOpName), kOpName,
                           opts.op_registry());
  node_builder.Input(input);
  node_builder.Input(filter);
...
Node* DepthwiseConv2dNative(NodeOut input, NodeOut filter, gtl::ArraySlice<int>
                            ...
                            GraphDefBuilder::Options& opts) {
  if (opts.HaveError()) return nullptr;
  static const string kOpName = "DepthwiseConv2dNative";
  NodeBuilder node_builder(opts.GetNameForOp(kOpName), kOpName,
                           opts.op_registry());
  node_builder.Input(input);
  node_builder.Input(filter);
...
Node* DepthwiseConv2dNativeBackpropFilter(NodeOut input, NodeOut filter_sizes,
                                          ...
                                          GraphDefBuilder::Options& opts) {
  if (opts.HaveError()) return nullptr;
  static const string kOpName = "DepthwiseConv2dNativeBackpropFilter";
  NodeBuilder node_builder(opts.GetNameForOp(kOpName), kOpName,
                           opts.op_registry());
  node_builder.Input(input);
  node_builder.Input(filter_sizes);
...
Node* DepthwiseConv2dNativeBackpropInput(NodeOut input_sizes, NodeOut filter,
                                         ...
                                         GraphDefBuilder::Options& opts) {
  if (opts.HaveError()) return nullptr;
  static const string kOpName = "DepthwiseConv2dNativeBackpropInput";
  NodeBuilder node_builder(opts.GetNameForOp(kOpName), kOpName,
                           opts.op_registry());
  node_builder.Input(input_sizes);
  node_builder.Input(filter);
...
Node* LogSoftmax(NodeOut logits, const GraphDefBuilder::Options& opts) {
  static const string kOpName = "LogSoftmax";
...
Node* MaxPool3D(NodeOut input, gtl::ArraySlice<int> ksize, gtl::ArraySlice<int>
                ...
                opts) {
  if (opts.HaveError()) return nullptr;
  static const string kOpName = "MaxPool3D";
  NodeBuilder node_builder(opts.GetNameForOp(kOpName), kOpName,
                           opts.op_registry());
  node_builder.Input(input);
  node_builder.Attr("ksize", ksize);
...
Node* MaxPool3DGrad(NodeOut orig_input, NodeOut orig_output, NodeOut grad,
                    ...
                    {
  if (opts.HaveError()) return nullptr;
  static const string kOpName = "MaxPool3DGrad";
  NodeBuilder node_builder(opts.GetNameForOp(kOpName), kOpName,
                           opts.op_registry());
