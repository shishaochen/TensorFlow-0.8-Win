...
Node* DeleteSessionTensor(NodeOut handle, const GraphDefBuilder::Options& opts)
                          {
  static const string kOpName = "DeleteSessionTensor";
...
Node* GetSessionHandle(NodeOut value, const GraphDefBuilder::Options& opts) {
  static const string kOpName = "GetSessionHandle";
...
Node* GetSessionTensor(NodeOut handle, DataType dtype, const
  ...
  if (opts.HaveError()) return nullptr;
  static const string kOpName = "GetSessionTensor";
  NodeBuilder node_builder(opts.GetNameForOp(kOpName), kOpName,
                           opts.op_registry());
...
Node* QueueDequeueUpTo(NodeOut handle, NodeOut n, DataTypeSlice
                       component_types, const GraphDefBuilder::Options& opts) {
  if (opts.HaveError()) return nullptr;
  static const string kOpName = "QueueDequeueUpTo";
  NodeBuilder node_builder(opts.GetNameForOp(kOpName), kOpName,
                           opts.op_registry());
  node_builder.Input(handle);
  node_builder.Input(n);
